...
def fetch_prods(cat_id):
    return requests.get(f'https://mat.se/api/product/listByCategory?categoryId={cat_id}').json()

def get_cat_prods(prods, cat_id):
    return list(filter(lambda prod: cat_id in [cat_info['id'] for cat_info in prod['categories']], prods))
